...
def build_model(users_and_facs_df, travel_dict, users, facs, budget_factor=1.0, strict_assign_to_one=False,
    ...
    #m.tiers = pyo.Param(m.facs, initialize=lambda m, j: int(m.users_and_facs_df.at[j, 'Tier']))# tier element added
    ...
    #m.tier_zero_constraint = pyo.Constraint([j for j in m.facs if m.tiers[j] == 0 or m.tiers[j] == 1],
                                            #rule=tier_zero_cstr_rule)
